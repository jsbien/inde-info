		...
		for div in self.pages[self.i].children:
			...
			for word in div.children:
				...
				if True:
					...
					height = (word.bbox[3] - word.bbox[1])/SCALE
					if word.len == 0:
						word.len = 0.01
					width = (word.bbox[2] - word.bbox[0])/(SCALE * word.len)
					font = dc.GetFont()
					if height != 0:
						font.SetPixelSize((width, height))
					dc.SetFont(font)
...
class MainWindow(wx.Frame):
	...
	def __init__(self, parent, title):
		...
		self.__toolbar.AddLabelTool(wx.ID_PREVIEW_PREVIOUS, "", wx.Bitmap("prev.png"))
		...
		self.__toolbar.AddLabelTool(wx.ID_PREVIEW_NEXT, "", wx.Bitmap("next.png"))
